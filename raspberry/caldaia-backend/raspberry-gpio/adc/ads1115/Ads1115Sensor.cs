...
namespace raspberry_gpio;
    ...
    /// </summary>
    public sealed class ADS1115Sensor : IDisposable
    {
        #region Fields
        ...
        /// </summary>
        /// <returns></returns>
        public void Initialize(int busId)
        {
            if (IsInitialized)
            {
            ...
            I2cBus controller = I2cBus.Create(busId);

            var settings = new I2cConnectionSettings(busId,ADC_I2C_ADDR);
            // gets the I2CDevice from the controller using the connection setting
            adc = controller.CreateDevice(ADC_I2C_ADDR);
